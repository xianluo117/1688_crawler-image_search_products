...
2. 其他应用通过带 API 密钥的 HTTP 接口上传图片，获取 1688 图片搜索链接。
...
1. 打开 `userscripts/ali1688-cookie-sync.user.js`。
2. 把 `@connect sync.example.com` 改为实际 HTTPS 域名，只填写域名。
3. 安装脚本。
4. 打开并登录 `https://www.1688.com/`。
5. 从脚本菜单执行“配置同步接口”。
6. 输入 `https://实际域名/api/cookie-sync`。
7. 执行“配置共享密钥”。
8. 输入 `.env` 中的 `COOKIE_SYNC_SHARED_SECRET`。
9. 执行“同步 1688 Cookie 到 Linux”。
...
测试使用假上传器，不请求真实 1688 接口，不使用真实 Cookie。
